...
import { ArrowRight, MoreVertical, Edit, Trash2, Copy, Share2, Tag as TagIcon, Check, Star, ChevronUp, Type } from 'lucide-react';
...
export const EducationReadingView: React.FC<EducationReadingViewProps> = ({
  ...
  const renderFormattedText = (rawText: string) => {
    ...
    const getTextSizeClass = (size: TextSize) => {
      switch (size) {
        case 'sm': return { heading: 'text-base sm:text-lg', paragraph: 'text-sm sm:text-base leading-relaxed sm:leading-loose' };
        case 'base': return { heading: 'text-lg sm:text-xl', paragraph: 'text-base sm:text-lg leading-[1.8] sm:leading-[2]' };
        case 'lg': return { heading: 'text-xl sm:text-2xl', paragraph: 'text-lg sm:text-xl leading-[2] sm:leading-[2.2]' };
        case 'xl': return { heading: 'text-2xl sm:text-3xl', paragraph: 'text-xl sm:text-2xl leading-[2.2] sm:leading-[2.5]' };
      }
    };
    
    const { heading, paragraph } = getTextSizeClass(textSize);
    ...
    return paragraphs.map((para, index) => {
      ...
      if (isHeading) {
        ...
        return (
          <h3
            key={index}
            className={`${heading} font-bold text-primary-theme mt-6 mb-2 border-b border-neutral-200/50 dark:border-neutral-800/50 pb-1.5`}
          >
            {cleanHeading}
          </h3>
        );
      ...
      return (
        <p key={index} className={`${paragraph} text-primary-theme mb-4`}>
          {trimmed}
        </p>
      );
    });
                ...
                <div className="px-3 py-2 border-b border-neutral-200/60 dark:border-neutral-800/60 mb-1">
                  ...
                  <div className="flex items-center justify-between gap-1">
                    {(['sm', 'base', 'lg', 'xl'] as TextSize[]).map((size) => (
                      <button
                        key={size}
                        onClick={() => { setTextSize(size); setIsMenuOpen(false); }}
